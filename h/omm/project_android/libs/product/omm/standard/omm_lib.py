    ...
    try:
        ...
        if g.devicetype == 'real':
            ...
            for gm_deviceId, gm_deviceName in gennymotion_device_dict.iteritems():
                ...
                print "g.device name is ############ %s" % str(g.device_name)   
                if str(gm_deviceName).strip() == str(g.device_name).strip():
                    print "YES"
...
def uninstall_android_omm_app():
    ...
    try:
        ...
        if get_android_installed_packages().__contains__(g.app_package):
            ...
            # changing device id runtime
            uninstall_cmd = g.android_uninstall_cmd.replace('device_id', g.android_device_id)
