...
class DellTestCase(TestCase):
    ...
    def set_step_message(self, message):
        """
        Set step message for PASS testcase.
        """
    ...
    def add_step_result(self, stepnum, name, description, result):
        if name:
            name = str(name).strip()
        if description:
    ...
    def run_steps(self, *steps, **kwargs):
        ...
        for num, step in enumerate(steps, 1):
            self.step_message = ''
            self.logger.info('Step %s Started.' % num)
            stepnumbers = re.findall('\d+', step.func_name)
            stepnum = (".".join(stepnumbers) if (stepnumbers) else num)
            try:
                output.append(step(output, step_args))
            except AssertionError, err:
                self.add_step_result(stepnum, self.shortDescription(step.func_doc), err, 'fail')
                ...
                self.add_step_result(stepnum, self.shortDescription(step.func_doc), reason, skip_type)
                if continue_on_fail is True:
                    self.logger.warn('Step %s %s: %s', num, skip_type.lower(), reason)
                else:
                    raise
            else:
                self.add_step_result(stepnum, self.shortDescription(step.func_doc), self.step_message, 'pass')
            self.logger.info('Step %s Completed.' % num)
