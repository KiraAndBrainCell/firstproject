...
def logs_folder():
    ...
    logs_folder = str(config.get('dellunit', 'logfolder'))
    return logs_folder
...
class TestResult(result.TestResult):
    ...
    def get_safe_filename(self, basename):
        ...
        logfile = os.path.join(logs_folder(), basename)
        while os.path.exists(logfile):
            count = count + 1
            logfile = os.path.join(logs_folder(), '%s_%02d%s' % (start, count,
                                   ext))
        return logfile
    ...
    def add_result(self, status, test, stdout, stderr):
        ...
        RESULT_FILE = (os.path.join(logs_folder(), 'testdata_%s.json'%self.logtime) if (self.timestamp_report) else os.path.join(logs_folder(), 'testdata.json'))
