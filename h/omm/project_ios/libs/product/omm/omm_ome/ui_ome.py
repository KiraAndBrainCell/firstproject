...
def click_add_ome():
    ...
    try:
        ...
        status = False
    return status, msg
def eenter_ome_details(omeip, ome_usrname, ome_pasword):
    """
    ...
    try:
        ...
        print 'plus button clicked'
            #flag2 =   ui_controls.button(get_obj_identifier('op
        #print 'openManage Essentials selected successfully'
        'Enter OME application ip and credentials'
        flag1 = ui_controls.text_box(get_obj_identifier('addOme_ipHostname_txt'),omeip)
        flag2 = ui_controls.text_box(get_obj_identifier('addOme_username_txt'),ome_usrname)
        flag3 = ui_controls.text_box(get_obj_identifier('addOme_password_txt'),ome_pasword)
        ui_controls.hide_keyboard()
        time.sleep(5)
        'Click on + button'
        time.sleep(5)
        flag4 =ui_controls.Click(get_obj_identifier('sve_opt'))
        print 'add ome button clicked'
        flag5 = ui_controls.Click(get_obj_identifier('addome_confirmation_yes_btn'))
        time.sleep(2)
         
        flag6 = ui_controls.Click(get_obj_identifier('Ok_bttn'))
        time.sleep(6)
        
        #flag7 =ui_controls.Click(get_obj_identifier('alert_ok_btn'))
        status = False if not (flag0 and flag1 and flag2 and flag3 and flag4 and flag5 and flag6) else True
    except Exception as excp:
        traceback.print_exc()
        msg += str(excp)
        status = False
...
def jaff_a():
    ...
    try:
        ...
        if g.platform == 'android':
            ...
            flag = False if not (flag1 and flag2 and flag4) else True
        else:
...
def add_ome_app(ome_ip, ome_username, ome_password):
    ...
    try:
       ...
       # flag3 = accept_ssl_certificate()
        ...
        status = False
    return status, msg
def add_ome_device(ome_ip, ome_username, ome_password):
    """
    This function is used for adding OME application
    ...
    try:
        ...
        'Enter OME details'
        flag2 = enter_ome_values(ome_ip, ome_username, ome_password)
