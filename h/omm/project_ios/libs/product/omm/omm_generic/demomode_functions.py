...
def verify_demomode_first_ome_devices_text():
    ...
    try:
        if g.platform == 'android':
            ...
            flag20 = verify_first_ome_device_devices(ntwrk_dvce='Network Devices',rac='RAC',servers='Servers')
            ...
            status = False if not (flag1 and flag2 and flag3 and flag16 and flag5 and flag17 and flag7 and flag18 and flag9 and flag10 and flag19 and flag20 and flag21 and flag22 and flag23 and flag24 and flag25 and flag26) else True
...
def verify_enable_demomode_option():
    ...
    try:
        if g.platform == 'android':
            #'click on menu to get setting option'
            #flag1 = ui_controls.Click(get_obj_identifier('DemoMode_menu'))
            #print "clicked on menu button to get settings page"
            #sleep(4)
            #'Click on setting option in menu'
            #flag2 = ui_controls.Click(get_obj_identifier('DemoMode_setting_option'))
            #print "clicked on settings page"
            #sleep(3)
            flag1=go_to_page_name("Settings")

           
            'verify  bar title text when demo mode is enabled'
            #print "verify the page labels  when demo mode is enabled"
            #flag3,msg = ui_generic.element_textvalidation('DemoMode_enable_Bar_Tittle','DEMO MODE SETTINGS')
              
            'swipe down to visible enable/disable toggle button'
            ...
            #print "verify page labels when demo mode is disabled"   
            #'verify  bar title text when demo mode is disabled'
            #flag6,msg = ui_generic.element_textvalidation('DemoMode_Disable_bar_Tittle','MISC SETTINGS')
            #sleep(4)     
            ...
            print "clicked on enable/disable demo mode toggle switch to disabled demo mode"
            
            #sleep(3)
            #print "verify the page labels  when demo mode is enabled"
            #flag10,msg = ui_generic.element_textvalidation('DemoMode_enable_Bar_Tittle','DEMO MODE SETTINGS')
            
            sleep(3)

            status = False if not (flag1 and flag2 and flag3 and flag4 and 
                                   flag5 and flag6) else True
        else:
...
def duplicate_verify_first_ome_device_home_page_txt(ome_dvce_to_click=None,device_name=None,ome_device_name=None,ome_device_home=None,devices_elemnt=None,devices_count=None,device_crticl_elemnt=None,
    ...
    try:
        if g.platform == 'android':
                ...
                'Comparing text retrieved from UI with verification input text'
                if text_to_verify.strip() == actual_text.strip():   
                        print " first Ome device  home page  text verified successfully,ome device name is"+str(device_name.strip())

                else:
                        print "first ome device home page text verified unsuccessfully,ome device name is"+str(device_name.strip())
                sleep(2)
...
def verify_first_ome_rac_devices_count():
    ...
    try:
        if g.platform == 'android':
                ...
                for txt in dict:
                    util.write_file(g.dynamic_first_ome_RAC_devices_txt, txt)
