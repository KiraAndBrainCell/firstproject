...
def logs_folder():
    ...
    print "Report json logs folder is %s" % logs_folder
    return logs_folder

def data_reader(logtime, timestamp_report=False):
    INPUT_FILE = (os.path.join(logs_folder(), 'testdata_%s.json'%logtime) if (timestamp_report) else os.path.join(logs_folder(), 'testdata.json'))
