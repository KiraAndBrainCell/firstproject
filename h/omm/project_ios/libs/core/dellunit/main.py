...
def logs_folder():
    ...
    print "Logs folder is %s" % logs_folder
    return logs_folder
...
def setup_logging(logtime, timestamp_report=False):
    ...
    logfile = (os.path.join(logs_folder(), 'runtime_%s.log'%logtime) if (timestamp_report) else os.path.join(logs_folder(), 'runtime.log'))
...
def wipe_logfiles(exceptions):
    ...
    for filename in os.listdir(logs_folder()):
        full_filename = os.path.join(logs_folder(), filename)
        try:
            del_file(filename)
        except Exception, e:
